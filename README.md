...
在目标服务器上执行一条命令即可完成安装，Token 自动生成：

```bash
curl -sSL https://raw.githubusercontent.com/leo88188/openclaw-agent/main/install-remote.sh | bash
```

指定端口：

```bash
curl -sSL https://raw.githubusercontent.com/leo88188/openclaw-agent/main/install-remote.sh | bash -s -- --port 9966
```
